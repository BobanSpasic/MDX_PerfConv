 ...
 *****************************************************************************
 ...
 first half (1 to 32).
 DX1 and DX5 uses 64 slots in combination with 4 voice banks.
...
type
  ...
  public
    ...
    function GetFunctionName(aNr: integer): string;
...
function TTX7FunctBankContainer.GetFunctionName(aNr: integer): string;
begin
  if (aNr > 0) and (aNr < 65) then
    Result := FTX7FunctBankParams[aNr].GetFunctionName
  else
    Result := '';
end;
...
procedure TTX7FunctBankContainer.SysExFunctBankToStream(aCh: integer;
...
procedure TTX7FunctBankContainer.AppendSysExFunctBankToStream(aCh: integer;
  var aStream: TMemoryStream);
var
  i: integer;
  FCh: byte;
