...
type
  ...
  public
    ...
    procedure LoadVoiceToTG(aNr: integer; aVCED: TDX7_VCED_Params);
...
begin
  ...
  begin
    ...
    begin
      ...
      VoiceNumber := 1;
      MIDIChannel := 255;
      GetDefinedValues(DX7, fInit, VoiceData.params);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        for j := 0 to 155 do
          VoiceData.params[j] := aStream.ReadByte;
        for j := 0 to 52 do
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        for j := 0 to 155 do
          aStream.WriteByte(VoiceData.params[j]);
        for j := 0 to 52 do
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FMDX_Params.TG[iCount].VoiceData.params[i] := byte(Hex2Dec(hexchar));
      end;
    end
    else
      GetDefinedValues(DX7, fInit, FMDX_Params.TG[iCount].VoiceData.params);
...
begin
  ...
  begin
    sCount := IntToStr(iCount);
    ini.WriteInteger('BankNumber' + sCount,
      FMDX_Params.TG[iCount].BankNumberMSB*127 + FMDX_Params.TG[iCount].BankNumberLSB);
    ini.WriteInteger('VoiceNumber' + sCount,
      FMDX_Params.TG[iCount].VoiceNumber);
    ...
    for i := 0 to 155 do
    begin
      hexchar := IntToHex(FMDX_Params.TG[iCount].VoiceData.params[i], 2);
...
procedure TMDXPerformanceContainer.LoadVoiceToTG(aNr: integer; aVCED: TDX7_VCED_Params);
begin
...
begin
  Result := '';
  for i := 145 to 154 do
    Result := Result + chr(FMDX_Params.TG[aNr].VoiceData.params[i]);
end;

function TMDXPerformanceContainer.GetTGVoiceData(aNr: integer): TDX7_VCED_Params;
begin
  FDX7_Params.params := FMDX_Params.TG[aNr].VoiceData.params;
  Result := FDX7_Params;
end;
...
begin
  ...
  for i := low(FMDX_Params.TG[aNr].VoiceData.params)
    to high(FMDX_Params.TG[aNr].VoiceData.params) do
    aStream.WriteByte(FMDX_Params.TG[aNr].VoiceData.params[i]);
